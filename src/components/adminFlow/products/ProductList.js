...
import { faFilter,faSlidersH, faSort, faClone, faEye, faEyeSlash, faRotateLeft } from '@fortawesome/free-solid-svg-icons';
...
const controllerRef = useRef(null);
  ...
  const handleCategoryChange = async (event) => {
    ...
    if (selectedCategoryId !== '') {
      ...
      fetchData({ category_id: selectedCategoryId,search:searchQuery, pg: 1 });
       const params = {
  ...
  pg: 1
    ...
    else{ fetchData({ search:searchQuery, pg: pageFromUrl }); }
  };
  ...
  const handleVariantChange = async (event) => {
    ...
    const selectedCategoryId = event.target.selectedOptions[0].dataset.categoryId;
    setSelectedCategoryId(selectedCategoryId);
    setSelectedVariant(selectedVariantId);
    if (selectedVariantId !== '') {
     const params = {
  ...(selectedCategory || selectedCategoryId && { category_id: selectedCategory || selectedCategoryId }),
  ...(selectedBrand && { brand_id: selectedBrand }),
...
const handleCombinationSelectCategory = async () => {
 ...
 if (searchQuery !== '') {
  ...
  filter: true,
  pg: 1,
  ...(selectedBrand || BrandId && { brand_id: selectedBrand || BrandId }),
  ...(selectedCategory && { category_id: selectedCategory }),
  ...(selectedVariant && { variant_option_name_id: selectedVariant }),
};
...
const fetchSearchResults = async (query) => {
  ...
  const selectedVariantData = variants.find(variant => variant.varient_option_id === selectedVariant);  
    ...
    <div className="product-list">
      <div className="search-container" style={{position:'relative'}}>
        ...
        {searchQuery && (
         ...
         style={{ position: 'absolute', right: '266px', background: 'transparent', border: 'none', fontSize: '16px', color: '#aaa', cursor: 'pointer', width: '15%', top:'-3px'
         }} > ✕ </button>
      )}
       <div className="count-vendor" style={{ display: 'flex', alignItems: 'center',flexDirection: 'row', justifyContent: 'flex-end', width: '32%' }}>
      ...
      <div className="sort-container">
        <select  value={selectedCategory}  onChange={handleCategoryChange}  className="filter-dropdown "  style={{margin:'8px 0px 7px 0px'}}>
                  <option value="">All Categories</option>
                  {categories.map((cat) => (
                    <option value={cat.id}>{cat.name}</option>   ))} </select>
 <select value={selectedBrand} onChange={handleBrandChange} className="filter-dropdown variant_dropdown">
                   <option value="">All Vendors</option>
                   {brands.map((brand) => (
                     <option  value={brand.id}>{brand.name}</option>
                   ))}
                 </select> 
            <select value={selectedVariant} onChange={handleVariantChange} className="filter-dropdown variant_dropdown half-width truncate-option">
            <option value="">All Variants</option>
            {variants.map((variant) => (
              <option value={variant.varient_option_id}  data-category-id={variant.category_id}>
                {variant.type_name}
              </option>
            ))}
          </select>
  ...
  <button onClick={handleFiltersClear} className="filters-clear-btn">
    <FontAwesomeIcon icon={faRotateLeft} />
        ...
        <select onChange={handleSortChange} value={sortOption} className="sort-dropdown variant_dropdown" style={{ cursor: 'pointer',}}>
        ...
        <table className="product-table">
          <thead>
            <tr>
              ...
              <th className="taxonomy-column" style={{width: '40%'}} onClick={() => handleSort("taxonomy")}>Taxonomy {sortColumn === "taxonomy" ? (sortOrder === "asc" ? "↑" : "↓") : ""}
          ...
          <select
            ...
            style={{ marginTop: '5px', width: '80%', display:'inline-block', fontSize: '12px' }}
          >
            <option value="" disabled>Change taxonomy for selected</option>
           ...
           <button onClick={handleClearSelected} className="products-clear-btn">
    <FontAwesomeIcon icon={faRotateLeft} />
  </button>
              </div>
        )}
         ...
         <div className="pagination-container">
            {totalPages > 1 && pageFromUrl > 1 && (
              <button className="pagination-button prev-button-plp" onClick={() => handlePageChange(pageFromUrl - 1)}>
                &laquo; Prev
              </button>
            )}
            {totalPages > 1 && (
            ...
            {totalPages > 1 && pageFromUrl < totalPages && (
              <button className="pagination-button next-button-plp" onClick={() => handlePageChange(pageFromUrl + 1)}>
                Next &raquo;
              </button>
            )}
          </div>
