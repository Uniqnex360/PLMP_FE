...
const PriceComponent = () => {
  ...
  const [variantpriceOption, setVariantPriceOption] =
    useState("finished_price");
  ...
  const [selectedBrandIdForVariant, setSelectedBrandIdForVariant] =
    useState(null);
  ...
  const handleCategorySelect = (e) => {
    ...
    if (categoryId === "all") {
      if (
        ...
        selectedCategories[0].name === "Apply to all categories"
      ) {
  ...
  const handleCategoryRemove = (categoryId) => {
    if (categoryId === "all") {
      ...
      setSelectedCategoryIds((prevSelectedCategoryIds) => {
        const newCategoryIds = prevSelectedCategoryIds.filter(
          (id) => id !== categoryId
        );
        fetchPriceTableData(newCategoryIds);
        return newCategoryIds;
  ...
  const handleBrandSelect = (brand) => {
    if (brand && brand.id) {
      ...
      const filtered=categories.filter((category)=>category.brand_id===brand.id)
      setFilteredCategories(filtered)
      setSelectedCategories([])
      setSelectedCategoryIds([])
    } else {
  ...
  const handleBrandSelectForVariant = (brand) => {
    ...
    if (brand && brand.id) {
      ...
      const filtered = variantOptions.filter(
        (variant) => variant.brand_id === brand.id
      );
  ...
  const handleVariantPriceChange = async (event) => {
    const selectedOption = event.target.value;
    if (selectedOption) {
      setVariantPriceOption(selectedOption);
    } else {
  ...
  const handlePriceApply = async () => {
    ...
    if (!selectedBrand || !selectedCategoryIds.length || !priceInput) {
      Swal.fire({
        text: "Please Enter all fields to apply",
        confirmButtonText: "OK",
        customClass: {
          container: "swal-custom-container",
          popup: "swal-custom-popup",
      ...
      try {
        ...
        const response = await axiosInstance.post(
          `${process.env.REACT_APP_IP}/updateRetailPrice/`,
          {
        ...
        if (response.status === 200) {
          Swal.fire({
            title: "Success",
            text: "Applied Based on Vendor & Category wise Successfully",
            icon: "success",
            confirmButtonText: "OK",
            customClass: {
              container: "swal-custom-container",
  ...
  const handleVariantPriceApply = async () => {
    ...
    if (
      ...
      try {
        setLoading(true);
        const response = await axiosInstance.post(
          `${process.env.REACT_APP_IP}/obtainProductBasedOnVarientOption/`,
          {
        ...
        if (response.status === 200) {
          Swal.fire({
            title: "Success",
            text: "Applied Based on Vendor & Variant wise Successfully",
            icon: "success",
            confirmButtonText: "OK",
            customClass: {
              container: "swal-custom-container",
        ...
        Swal.fire({
          ...
          didRender: () => {
            ...
            saveButton.style =
              "padding: 10px 20px; background-color: #4CAF50; color: white; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px;";
            saveButton.onclick = async () => {
              try {
                const saveresponse = await axiosInstance.post(
                  `${process.env.REACT_APP_IP}/saveChangesForVarientOption/`,
  ...
  const fetchPriceTableDataBrand = async (BrandID) => {
    try {
      ...
      const response = await axiosInstance.post(
        `${process.env.REACT_APP_IP}/obtainBrandCategoryWisePriceTable/`,
        payload
      );
      ...
      Swal.fire({
        title: "Error",
        text: "Failed to fetch price table data.",
        icon: "error",
        confirmButtonText: "OK",
        customClass: {
          container: "swal-custom-container",
  ...
  const fetchVariantOptions = async () => {
    try {
      const response = await axiosInstance.get(
        `${process.env.REACT_APP_IP}/obtainVarientOptionForRetailPrice/`
      );
  ...
  useEffect(() => {
    const defaultVariant = variantOptions?.find(
      (variant) => variant.name.toLowerCase() === "wood type"
    );
    if (defaultVariant) {
  ...
  const handleVariantSelect = async (id) => {
    const variant =
      selectedBrandIdForVariant && filteredVariantOptions.length > 0
    ...
    try {
      const response = await axiosInstance.get(
        `${process.env.REACT_APP_IP}/obtainVarientOptionValueForRetailPrice/?id=${id}`
      );
  ...
  const handleVariantRemove = () => {
    ...
    if (selectedBrandIdForVariant) {
      const filtered = variantOptions.filter(
        (variant) => variant.brand_id === selectedBrandIdForVariant
      );
      setFilteredVariantOptions(filtered);
    }
  };
  const handleVariantValueSelect = (event) => {
    ...
    if (selectedValueId === "all") {
      ...
      const selectedValue = variantTypeValues.find(
  ...
  const handleVariantValueRemove = (id) => {
    ...
    setSelectedVariantValueIds((prevSelectedValueIds) =>
      prevSelectedValueIds.filter((valueId) => valueId !== id)
    );
  };
  return (
    <div
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Based on Vendor & Categories
          </h4>
          <h3
            style={{ marginBottom: "8px", fontSize: "18px", fontWeight: "500" }}
          >
          ...
          <div>
            <select
              ...
              onChange={(e) =>
                handleBrandSelect(
                  brands.find((brand) => brand.id === e.target.value)
                )
              }
            >
            ...
            <span
              style={{
                ...
                right: "25px",
                fontSize: "12px",
                color: formSubmitted && !selectedBrand ? "red" : "#918f8f",
              }}
            >
              {" "}
              ▼{" "}
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: "5px",
                  border:
                    formSubmitted && selectedCategoryIds.length === 0
                      ? "1px solid red"
                      : "1px solid #ccc",
                  width: "225px",
                  cursor: "pointer",
              ...
              {dropdownOpen && (
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() =>
                      handleCategorySelect({ target: { value: "all" } })
                    }
                  >
                    ...
                    filteredCategories.map((category) => (
                      ...
                      >
                        {selectedCategoryIds.includes(category.id) && (
                          <span
                            style={{ marginRight: "8px", color: "#18b418" }}
                          >
                            ✔
                          </span>
                        )}
          ...
          <div style={{ margin: "20px 0px 0px 0px", width: "100%" }}>
            ...
            <div style={{ margin: "0px 0px 0px 0px", display: "inline-block" }}>
              ...
              >
                ...
                <option value="un_finished_price">
                  Unfinished Wholesale Price
                </option>
              </select>
              <span
                style={{
        ...
        >
          ...
          <div>
            <select
              style={{
                ...
                borderRadius: "5px",
                border:
                  formSubmittedForVariant && !selectedBrandForVariant
                    ? "1px solid red"
                    : "1px solid #ccc",
                width: "248px",
                display: "inline-block",
              ...
              <div style={{ marginTop: "10px", display: "inline-block" }}>
                ...
                >
                  ...
                  <span
                    ...
                    onClick={handleBrandRemoveForVariant}
                  >
                    {" "}
                    X{" "}
                  </span>
          ...
          <div style={{ marginTop: "20px" }}>
            ...
            <select
              style={{
                ...
                borderRadius: "5px",
                border:
                  formSubmittedForVariant && !selectedVariant
                    ? "1px solid red"
                    : "1px solid #ccc",
                width: "248px",
                display: "inline-block",
            ...
            >
              ...
              {(selectedBrandIdForVariant && filteredVariantOptions.length > 0
                ...
                <option
                  value={variant.id}
                  selected={
                    variant.name.toLowerCase() === "wood type" &&
                    !selectedVariant
                  }
                >
                  {variant.name}
          ...
          <div style={{ margin: "10px 0" }}>
            ...
            >
              Select Variant Value <span className="required">*</span>
            </h3>
            <div
              ref={dropdownRefForValue}
              style={{ position: "relative", display: "inline-block" }}
            >
              <div
                style={{
                  ...
                  borderRadius: "5px",
                  border:
                    formSubmittedForVariant && !selectedVariantValueIds.length
                      ? "1px solid red"
                      : "1px solid #ccc",
                  width: "225px",
                  cursor: "pointer",
              ...
              {dropdownOpenForValue && (
                ...
                >
                  <div
                    ...
                    onClick={() =>
                      handleVariantValueSelect({ target: { value: "all" } })
                    }
                  >
                  ...
                  {variantTypeValues.map((variant) => (
                    <div
                      style={{
                        ...
                        cursor: "pointer",
                        borderRadius: "4px",
                        background: selectedVariantValueIds.includes(variant.id)
                          ? "#d7ffe6"
                          : "#fff",
                        fontSize: "14px",
                        marginBottom: "2px",
                      ...
                      onClick={() =>
                        handleVariantValueSelect({
                          target: { value: variant.id },
                        })
                      }
                    >
                      {selectedVariantValueIds.includes(variant.id) && (
                        <span style={{ marginRight: "8px", color: "#18b418" }}>
                          ✔
                        </span>
                      )}
                      {variant.name}
              ...
              {selectedVariantValues.map((variant) => (
                ...
                >
                  ...
                  <span
                    ...
                    onClick={() => handleVariantValueRemove(variant.id)}
                  >
                    {" "}
                    X{" "}
                  </span>
            ...
            >
              <input
                ...
                style={{
                  width: "70%",
                  border:
                    formSubmittedForVariant && !VariantpriceInput
                      ? "1px solid red"
                      : "1px solid #ccc",
                }}
              />
          ...
          <button
            onClick={() => handleVariantPriceApply()}
            className="apply-btn"
          >
            {" "}
            Apply{" "}
          ...
          <p style={{ display: "contents" }}>
            (This given variant price will update the variant price and the
            retail price accordingly) <span className="required">*</span>
          </p>
          {(selectedBrandForVariant ||
            selectedVariantValueIds.length > 0 ||
            VariantpriceInput.length > 0) && (
            <button
              onClick={handleVarianyPriceApplyClear}
              className="clear-btn"
            >
              {" "}
              Clear all{" "}
      ...
      <div>
        {productTableDataAfterSave.length > 0 && (
          <>
            ...
            >
              Total Products: {productcountDataAfterSave}
            </span>
            <table
              border="1"
              style={{ marginTop: "20px", width: "100%", textAlign: "left" }}
