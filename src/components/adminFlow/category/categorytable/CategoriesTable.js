...
const handleLevelClear = (e) => {
  ...
  navigate(`/Admin/categorylist?categoryId=${selectedCategoryIdForallprod}&level=${selectedCategorylevelForallprod}&page=${page}`);
       const params = {
                category_id: selectedCategoryIdForallprod,
                level_name: selectedCategorylevelForallprod,
                filter: true,
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0px' }}>
            ...
            <h3 style={{ fontWeight: 'bold', marginLeft: '10px' }}>
              ...
              <FontAwesomeIcon
                ...
                style={{ cursor: 'pointer', fontSize: '18px', marginRight: '10px' }}
              /> Total Products: <span className="brand-count"> {productsCount}</span> </h3>
          </div>
            ...
            <Table>
              <TableHead style={{background:'#e9e9e9'}}>
                <TableRow>
                  ...
                  <TableCell align="left" sx={{ fontWeight: 'bold', fontSize: '14px', padding: '10px', cursor: 'pointer', width:'40%' }} onClick={() => handleSort('product_name')} >
                    Product Name
  ...
  {loading ? (
  <TableRow>
    <TableCell colSpan={6} sx={{ textAlign: 'center', padding: '20px', fontSize: '16px', color: '#888' }}>
      Products Loading...
    </TableCell>
  </TableRow>
